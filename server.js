...
function saveData(data) {
  try {
    const volCount = (data.volumeData || []).length;
    const slaCount = (data.slaData || []).length;
    console.log(`[SAVE] Writing data.json — volumeData: ${volCount}, slaData: ${slaCount}`);
    fs.writeFileSync(DB_FILE, JSON.stringify(data, null, 2), 'utf8');
    console.log(`[SAVE] ✅ data.json saved successfully`);
    return true;
  } catch (error) {
...
app.post('/api/upload-sla', upload.single('file'), (req, res) => {
  ...
  try {
    ...
    Object.values(trips).forEach(trip => {
      ...
      db.slaData.push({ id: db.slaData.length + 1, ...trip, category: category || 'primer', weekStart: week.startDate, weekEnd: week.endDate, weekKey: week.weekKey, uploadTime: new Date().toISOString() });
      successCount++;
    });
...
app.get('/api/volume', (req, res) => {
  ...
  if (category) {
    ...
    filtered = filtered.filter(item => item.category && item.category.toLowerCase() === catLower);
  }
...
app.get('/api/sla', (req, res) => {
  ...
  try {
    ...
    if (category) {
      const catLower = category.toLowerCase();
      filtered = filtered.filter(item => item && item.category && item.category.toLowerCase() === catLower);
    }
...
app.delete('/api/sla', (req, res) => {
  try {
    const { routeName, routeNames, category } = req.body;
    ...
    db.slaData = db.slaData.filter(item => {
      // If a specific category is requested, only delete matching category
      if (catLower && item.category && item.category.toLowerCase() !== catLower) return true;
      return !targetRoutes.includes(item.rute);
    });
    ...
    console.log(`Deleted ${deletedCount} SLA records for route(s) in category ${category || 'all'}`);
    res.json({ 
      success: true, 
      message: `Successfully deleted ${deletedCount} SLA records` 
    });
...
app.delete('/api/volume', (req, res) => {
  try {
    ...
    const targetRoutes = routeNames || [routeName];
    const catLower2 = category ? category.toLowerCase() : null;
    
    // Find and remove Volume data for the specified route(s)
    const originalLength = db.volumeData.length;
    db.volumeData = db.volumeData.filter(item => {
      // If a specific category is requested, only delete matching category
      if (catLower2 && item.category && item.category.toLowerCase() !== catLower2) return true;
      return !targetRoutes.includes(item.rute);
    });
    ...
    console.log(`Deleted ${deletedCount} Volume records for route(s) in category ${category || 'all'}`);
    res.json({ 
      success: true, 
      message: `Successfully deleted ${deletedCount} Volume records` 
    });
...
app.put('/api/volume', (req, res) => {
  try {
    const { rute, postal, nonPostal, tanggal, id, category } = req.body;
    ...
    db.volumeData = db.volumeData.map(item => {
      ...
      if (id) {
        ...
        match = (item.rute === rute && (!tanggal || item.tanggal === tanggal));
...
app.put('/api/sla', (req, res) => {
  try {
    const { rute, titik, tanggal, id, category } = req.body;
